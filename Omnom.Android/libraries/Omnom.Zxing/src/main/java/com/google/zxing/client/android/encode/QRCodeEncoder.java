...
final class QRCodeEncoder {
	...
	private void encodeQRCodeContents(Intent intent, String type) {
		...
		if (type.equals(Contents.Type.CONTACT)) {
			...
			if (bundle != null) {
				...
				String[] encoded = encoder.encode(Collections.singletonList(name), organization, Collections.singletonList(address),
				                                  phones,
				                                  phoneTypes, emails, urls, note);
	...
	private void encodeQRCodeContents(AddressBookParsedResult contact) {
		...
		String[] encoded = encoder.encode(toList(contact.getNames()), contact.getOrg(), toList(contact.getAddresses()),
		                                  toList(contact.getPhoneNumbers()), null, toList(contact.getEmails()), toList(contact.getURLs()),
		                                  null);
		// Make sure we've encoded at least one field.
		if (!encoded[1].isEmpty()) {
			contents = encoded[0];
