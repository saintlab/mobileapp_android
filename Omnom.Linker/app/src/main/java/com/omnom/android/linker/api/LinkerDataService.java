...
public interface LinkerDataService {
	...
	@FormUrlEncoded
	@POST("/authenticate")
	Observable<String> authenticate(@Field(ApiProtocol.FIELD_USERNAME) String username,
	                                @Field(ApiProtocol.FIELD_PASSWORD) String password);
	...
	@GET("/checkBeacon")
	Observable<Integer> checkBeacon(@Query(ApiProtocol.FIELD_RESTAURANT_ID) String restaurantId,
	                                @Query(ApiProtocol.FIELD_BEACON_UUID) String beaconUuid,
	                                @Query(ApiProtocol.FIELD_MAJOR_ID) String majorId,
	                                @Query(ApiProtocol.FIELD_MINOR_ID) String minorId);

	@POST("/ibeacons/bind")
	Observable<Integer> bindBeacon(@Field(ApiProtocol.FIELD_RESTAURANT_ID) String restaurantId,
	...
	@POST("/commitBeacon")
	Observable<Integer> commitBeacon(@Field(ApiProtocol.FIELD_RESTAURANT_ID) String restaurantId,
	                                 @Field(ApiProtocol.FIELD_BEACON_UUID) String beaconUuid,
	                                 @Field(ApiProtocol.FIELD_MAJOR_ID) String majorId,
	                                 @Field(ApiProtocol.FIELD_MINOR_ID) String minorId);

	@GET("/checkQrCode")
	Observable<Integer> checkQrCode(@Query(ApiProtocol.FIELD_RESTAURANT_ID) String restaurantId,
	                                @Query(ApiProtocol.FIELD_QR_DATA) String qrData);

	@POST("/bindQrCode")
	Observable<Integer> bindQrCode(@Field(ApiProtocol.FIELD_RESTAURANT_ID) String restaurantId,
	                               @Field(ApiProtocol.FIELD_TABLE_NUMBER) int tableNumber,
	                               @Field(ApiProtocol.FIELD_QR_DATA) String qrData);
